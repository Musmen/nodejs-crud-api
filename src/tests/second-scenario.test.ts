...
describe('Second test scenario', () => {
  ...
  test('Should answer with status code 400 and corresponding message if userId is invalid in GET, DELETE or PUT requests', async () => {
    ...
    const incorrectUserID = 123456789;
    const getResponse = await request
      .get(`${ENDPOINTS.USERS}/${String(incorrectUserID)}`)
      .expect(STATUS_CODES.BAD_REQUEST);
    expect(getResponse.text).toBe(RESPONSE_MESSAGES.BAD_REQUEST);

    const deleteResponse = await request
      .delete(`${ENDPOINTS.USERS}/${String(incorrectUserID)}`)
      .expect(STATUS_CODES.BAD_REQUEST);
    expect(deleteResponse.text).toBe(RESPONSE_MESSAGES.BAD_REQUEST);

    const putResponse = await request
      .put(`${ENDPOINTS.USERS}/${String(incorrectUserID)}`)
      .expect(STATUS_CODES.BAD_REQUEST);
    expect(putResponse.text).toBe(RESPONSE_MESSAGES.BAD_REQUEST);

    const responseAfterAll = (await request
      .get(ENDPOINTS.USERS)
      .expect(STATUS_CODES.OK)) as UsersResponse;
    expect(responseAfterAll.body).toHaveLength(1);

    const correctGetResponse = (await request
      .get(`${ENDPOINTS.USERS}/${createdUserId}`)
      .expect(STATUS_CODES.OK)) as UsersResponse;
    expect(correctGetResponse.body).toMatchObject({ ...newUser, id: createdUserId });
  });

  test("Should answer with status code 404 and corresponding message if there is no user with userId in users's database GET, DELETE or PUT requests", async () => {
    const response = (await request.get(ENDPOINTS.USERS).expect(STATUS_CODES.OK)) as UsersResponse;
    expect(response.body).toHaveLength(0);
    ...
    const notFoundUserIDChars = createdUserID.split('');
    notFoundUserIDChars[0] = createdUserID.startsWith('0') ? '1' : '0';
    const notFoundUserID = notFoundUserIDChars.join('');

    const getResponse = await request
      .get(`${ENDPOINTS.USERS}/${notFoundUserID}`)
      .expect(STATUS_CODES.NOT_FOUND);
    expect(getResponse.text).toBe(RESPONSE_MESSAGES.NOT_FOUND);

    const deleteResponse = await request
      .delete(`${ENDPOINTS.USERS}/${notFoundUserID}`)
      .expect(STATUS_CODES.NOT_FOUND);
    expect(deleteResponse.text).toBe(RESPONSE_MESSAGES.NOT_FOUND);

    const putResponse = await request
      .put(`${ENDPOINTS.USERS}/${notFoundUserID}`)
      .expect(STATUS_CODES.NOT_FOUND);
    expect(putResponse.text).toBe(RESPONSE_MESSAGES.NOT_FOUND);
